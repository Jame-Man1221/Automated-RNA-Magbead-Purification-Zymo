''' This file is written for ease of simulation. It has Opentrons OT-2 compatible labware loaded into the script such that 
the protocol can be simulated in applicable software (Spyder, VS code, Mac Terminal etc.) with the "opentrons_simulate.exe file_name.py" command. 
Timers have been changed to only run for 0.1 seconds - otherwise the script will loop tens of thousands of times during any 'while' loops '''
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    #times changed to 0.1s for simulation so it doesn't loop tens of thousands of times in the runlog
    five_mins=time.time()+0.1
    ten_mins=time.time()+0.1
    twenty_mins=time.time()+0.1
    ...
    # Load labware
    #reservoirs
    res1 = protocol.load_labware("nest_12_reservoir_15ml", 1)  
    ...
    prep_plate = protocol.load_labware('nest_96_wellplate_2ml_deep', 8)
